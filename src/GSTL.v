...
Inductive GSTL : list form -> option form -> Prop :=
  ...
  | La1:    forall G D (a b : form), a::G ⇒g D -> (a ∧ b)::G ⇒g D
  | La2:    forall G D (a b : form), b::G ⇒g D -> (a ∧ b)::G ⇒g D
  | Ra:     forall G (a b : form), G ⇒g a -> G ⇒g b -> G ⇒g (a ∧ b)
  | Lo:     forall G D (a b : form), a::G ⇒g D -> b::G ⇒g D -> (a ∨ b)::G ⇒g D
  | Ro1:    forall G (a b : form), G ⇒g a -> G ⇒g (a ∨ b)
  | Ro2:    forall G (a b : form), G ⇒g b -> G ⇒g (a ∨ b)
  | Li:     forall G D (a b : form), G ⇒g a -> b::G ⇒g D -> ∇(a ⊃ b)::G ⇒g D
