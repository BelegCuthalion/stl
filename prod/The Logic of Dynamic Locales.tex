...
But if $m>0$, the cut-data could also be $(\nabla A, m-1, \{n_i\})$, in which case using the induction hypothesis would suffice.
 ...
 First, notice that $m > 1$ is impossible, since the only rules that introduce $\nabla$ to the right of the sequence are $Fa$ for $\mathbf{LDL}$ based systems and $Fa_\rightsquigarrow$ for $\mathbf{LDL}_\rightsquigarrow$ based ones, which introduce a $\Box$ immediately after the $\nabla$ and implies $m = 1$. So suppose $m = 1$ and $\mathcal{D}_0$ end with $Fa$ (in $\mathbf{LDL}$).
 \begin{prooftree}
	 \AXC{$\mathcal{D}_0'$}
	 \noLine
	 \UIC{$\Gamma \Rightarrow A$}
	 ...
	 \UIC{$\Gamma \Rightarrow \nabla \Box A$}
 \end{prooftree}
 The cut-data must be of the form $(\Box A, 1, \{n_i\})$, so the only option for $\mathcal{D}_1$ is $L\Box$.
 \begin{prooftree}
	 \AXC{$\mathcal{D}_1'$}
	 \noLine
