...
\begin{proof}
	...
	II. For the other direction, using theorems \ref{thm:ldl-eq-ldls} and \ref{thm:ldls-cut-elim}, suppose $\Gamma \Rightarrow \Delta$ has a cut-free proof-tree in $\mathbf{LDL}^*_\rightsquigarrow(L)+A$, and it suffices to prove its equivalent $\nabla^n A, \Gamma \Rightarrow \Delta$ for some $n$ in $\mathbf{LDL}^*_\rightsquigarrow(L)$. By induction on the length of $\mathcal{D}$, we can assume that there is such equivalent in $\mathbf{LDL}^*_\rightsquigarrow(L)$ for the possible sub-proof(s) of $\mathcal{D}$.

	Now for different possible rules in $\mathbf{LDL}^*_\rightsquigarrow(L)+A$ as the last rule of $\mathcal{D}$, we will construct an $\mathbf{LDL}^*_\rightsquigarrow(L)$ proof of $\nabla^n A , \Gamma \Rightarrow \Delta$ for some $n$. In all cases, the possible immediate sub-trees of $\mathcal{D}$ are called $\mathcal{D}_0$ and $\mathcal{D}_1$.
	
	If $\mathcal{D}$ is just $\Rightarrow A$, then $\Gamma = \{\}$ and $\Delta = A$. We have $A \Rightarrow A$ by $Id$ with $n = 0$.

	If $\mathcal{D}$ is any of the axioms $Id$, $Ta$ or $Ex$, then we can add $A$ to the left side of the same axioms in $\mathbf{LDL}^*_\rightsquigarrow(L)$ to construct the desired sequent.
...
\end{thm}
\begin{proof}
	In case $\Delta' = \{\}$, just take $C = \bot$. Now let's assume $\Delta' = A$ for some formula $A$. We will prove the following stronger version: For any $\Gamma_1$, $\Gamma_2$ and $\Delta$, if $\vdash \Gamma_1 , \Gamma_2 \Rightarrow \Delta$ then there is a formula $C$ and a natural number $m$ such that
	\begin{enumerate}[label=(\arabic**)]
	...
	\begin{enumerate}
		...
		We also have $P(C_1) \subseteq P(\Gamma_1) \cap P(\Gamma_2 , A)$ and $P(C_2) \subseteq P(\Gamma_1) \cap P(\Gamma_2 , B)$. So $P(C_1 \land C_2) \subseteq P(\Gamma_1) \cap P(\Gamma_2 , A \land B)$ as it was justified before.
		...
		\begin{enumerate}
			...
			From IH, we also have $P(C_1) \subseteq P(\Gamma_1') \cap$ $P(\Gamma_2' , \nabla^n A)$ and $P(C_2) \subseteq P(\Gamma_1') \cap P(\Gamma_2' , \nabla^n B , \Delta)$. This implies $P(\nabla^{m_2''}(C_1 \land C_2)‎) \subseteq P(\Gamma_1') \cap P(\Gamma_2' , \nabla^{n+1} (A \rightsquigarrow B) , \Delta)$.
