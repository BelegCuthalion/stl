...
\subsubsection{Interpolation for $\mathbf{LDL}_\rightsquigarrow(L)$}
We can not prove the same result for the fragment $\mathbf{LDL}_\rightsquigarrow$, since we ought to use $\rightarrow$ to construct the interpolant in the case for $L\rightsquigarrow$. But if we add the rule $L$ to this fragment, we can prove a weaker form of interpolation using $rightsquigarrow$, as we will do in this section.
Throughout this section, when we use plain $\vdash$ symbol without specifying the deductive system, we mean deduction in $_{\mathbf{LDL}_\rightsquigarrow(L)}$. So by $\Gamma' \Rightarrow \Delta' \vdash \Gamma \Rightarrow \Delta$ we mean that $\Gamma \Rightarrow \Delta$ is provable in $\mathbf{LDL}_\rightsquigarrow(L)+\{\Gamma' \Rightarrow \Delta'\}$.
\begin{dfn}[Notation]\quad
	\begin{enumerate}
		\item Let $a$ be a finite sequence of natural numbers of length $k$ and $A$ be a formula. Recall that we use the abbreviation $\Box A = \top \rightsquigarrow A$. We will write $(\nabla\Box)^a A$ for the formula constructed from applying $a_k$ times $\Box$, $a_{k-1}$ times $\nabla$, ..., $a_2$ times $\Box$ and $a_1$ times $\nabla$ to $A$. More precisely
...
\begin{lem}
	\label{lem:box-rule} $A \Rightarrow B \vdash_{\mathbf{LDL}_\rightsquigarrow(\star)} \Box A \Rightarrow \Box B$.
\end{lem}
\begin{proof}\quad
	\begin{prooftree}
		\AXC{}
		...
		\AXC{$A \Rightarrow B$}
		\RightLabel{$L\rightsquigarrow$}
		\BIC{$\nabla (\top \rightsquigarrow A) \Rightarrow B$}
		\RightLabel{$Lw$}
		\UIC{$\nabla (\top \rightsquigarrow A) , \top \Rightarrow B$}
		\RightLabel{$R\rightsquigarrow$}
		\UIC{$\top \rightsquigarrow A \Rightarrow \top \rightsquigarrow B$}
	\end{prooftree}
\end{proof}

\begin{lem}
	\label{lem:nabla-box-rule} For any finite sequence of natural numbers $a$, $A \Rightarrow B \vdash_{\mathbf{LDL}_\rightsquigarrow(\star)} (\nabla\Box)^a A \Rightarrow (\nabla\Box)^a B$.
\end{lem}
\begin{proof}\quad
	\begin{prooftree}
		\AXC{} \noLine
...
\begin{lem}
	\label{lem:vdash} $\Rightarrow A \vdash \Gamma \Rightarrow \Delta$ if and only if $\vdash N(A, f) , \Gamma \Rightarrow \Delta$ for some $f$.
\end{lem}
\begin{proof}
	I. Suppose we have $\Rightarrow A$ and $N(A, f) , \Gamma \Rightarrow \Delta$ for some $f$. First for any $a \in f(A)$, construct $\Rightarrow \nabla^{a_1} \Box^{a_2} \dots \nabla^{a_{k-1}} \Box^{a_k} A$ from $\Rightarrow A$ as follows: Introduce $\top$ in the left side with $Lw$ and apply $R\rightsquigarrow$ to get $\Rightarrow \top \rightsquigarrow A$. Repeat $a_k$ times to get $\Rightarrow \Box^{a_k} A$. Then $a_{k-1}$ applications of $N$ yields $\Rightarrow \nabla^{a_{k-1}} \Box^{b_k} A$. Repeat this process for all elements of $a$, and do this for all $a \in f(A)$. Now $Cut$ all of them into $N(A, f) , \Gamma \Rightarrow \Delta$ to get $\Gamma \Rightarrow \Delta$. \\

	II. For the other direction, we can prove a stronger version: Suppose $\Rightarrow A \vdash \Gamma \Rightarrow \Delta$ by a proof tree $\mathcal{D}$. We want to prove $\vdash \{ \nabla^a \Box^b A\}_{(a,b) \in I} , \Gamma \Rightarrow \Delta$ for some finite set of pairs of natural numbers $I$. By induction on the length of $\mathcal{D}$, for any $\Gamma'$, $\Delta'$ and any $\mathbf{LDL}_\rightsquigarrow(L)+A$ proof tree $\mathcal{D}'$ which proves $\Gamma' \Rightarrow \Delta'$ in lower length than $\mathcal{D}$, we can assume there is an $\mathbf{LDL}_\rightsquigarrow(L)$ proof tree IH($\mathcal{D}'$) that proves $\{\nabla^a \Box^b A\}_{(a,b) \in I_{IH(\mathcal{D}')}} , \Gamma' \Rightarrow \Delta'$ for some set of pairs of natural numbers $I_{IH(\mathcal{D}')}$.

	For different possible rules in $\mathbf{LDL}_\rightsquigarrow(L)+A$ as the last rule of $\mathcal{D}$, we will construct an $\mathbf{LDL}_\rightsquigarrow(L)$ proof of $\{ \nabla^a \Box^b A\}_{(a,b) \in I} , \Gamma \Rightarrow \Delta$ for some finite set of pairs natural numbers $I$. In all cases, the possible immediate sub-trees of $\mathcal{D}$ are called $\mathcal{D}_0$ and $\mathcal{D}_1$.
	\begin{enumerate}
		\setcounter{enumi}{-1}
		...
		\item[4-11.] ($Lw$),($Lc$),($Rw$),($L\land_1$),($L\land_2$),($R\lor_1$),($R\lor_2$),($L$) Just apply the same rule on IH($\mathcal{D}_0$). Notice that $I$ would be the same set we get from the induction hypothesis.
		\setcounter{enumi}{11}

		\item ($N$) Same as the previous cases, except that $I = \{ (a + 1 , b) : (a , b) \in I_{IH(\mathcal{D}_0)} \}$, where $I_{IH(\mathcal{D}_0)}$ is the set from the induction hypothesis.

		\item ($Cut$) Just apply $Cut$ on IH($\mathcal{D}_0$) and IH($\mathcal{D}_1$). Notice that $I = I_{IH(\mathcal{D}_0)} \cup I_{IH(\mathcal{D}_1)}$.

		\item[14-16.] ($R\land$),($L\lor$),($L\rightsquigarrow$) Same as the $Cut$ case, except that before applying the corresponding rule to IH($\mathcal{D}_0$) and IH($\mathcal{D}_1$), we must first equalize their contexts using enough applications of $Lw$, to $\{ \nabla^a \Box^b A \}_{(a,b) \in I_{IH(\mathcal{D}_0)} \cup I_{IH(\mathcal{D}_1)}}$.
		\setcounter{enumi}{16}

		\item ($R\rightsquigarrow$) If $a = 0$ for some $(a,b) \in I_{IH(\mathcal{D}_0)}$, then $R\rightsquigarrow$ is not applicable on IH($\mathcal{D}_0$). We have $\nabla \Box^{b+1} A \Rightarrow \Box^b A$ from lemma \ref{lem:l-nabla-box}. For all such $(0,b) \in I_{IH(\mathcal{D}_0)}$ we first $Cut$ this into IH($\mathcal{D}_0$), so that the context has at least one $\nabla$ before applying $R\rightsquigarrow$. Notice that $I = \{ (a, b) : (a+1, b) \in I \} \cup$ $\{ (0, b+1) : (0, b) \in I \}$.
	\end{enumerate}
\end{proof}

\begin{thm}[Deductive Interpolation for $\mathbf{LDL}_\rightsquigarrow(L)$] For any $\Delta'$ and $\Delta$, if $\Rightarrow \Delta' \vdash \Rightarrow \Delta$ then there exists a formula $C$ such that
...
\end{thm}
\begin{proof}
	In case $\Delta' = \{\}$, just take $C = \bot$. Now let's assume $\Delta' = A$ for some formula $A$. We will prove the following stronger version: For any $\Gamma_1$, $\Gamma_2$ and $\Delta$, if $\vdash \Gamma_1 , \Gamma_2 \Rightarrow \Delta$ then there is a formula $C$ and a finite multi-set of formulas $\Pi$, consisting of members of $\Gamma_1$ with some $\nabla$ and $\Box$, such that
	\begin{enumerate}[label=(\arabic**)]
		\item $\vdash \Pi \Rightarrow C$,
		\item $\vdash \Gamma_2 , C \Rightarrow \Delta$,
	...
	\end{enumerate}
	Using the notation defined above we will omit (4*) and replace $\Pi$ in (1*) by $N(\Gamma_1, f)$ for some $f$.

	Then to prove the original statement, first convert the proof-tree for $\Rightarrow A \vdash \Rightarrow \Delta$ to a cut-free proof-tree for $\{\nabla^a\Box^b A : (a,b) \in I\} \Rightarrow \Delta$ in $\mathbf{LDL}^*_\rightsquigarrow(L)$ using lemma \ref{lem:vdash} and theorems \ref{thm:ldls-cut-elim} and \ref{thm:ldl-eq-ldls}, and then taking $\Gamma_1 = \{\nabla^a \Box^b A : (a,b) \in I \}$, $\Gamma_2 = \{\}$ proves the original statement of the theorem. Notice that we can again translate (2*) and (3*) to (2) and (3) respectively, using \ref{lem:vdash}, \ref{thm:ldls-cut-elim} and \ref{thm:ldl-eq-ldls}. To convert (1*) $\vdash \{\nabla^a \Box^b \nabla^c \Box^d A : (a,b) \in I , (c,d) \in J \} \Rightarrow C$ to to (1) $\Rightarrow A \vdash C$ we must construct $\Rightarrow \nabla^a \Box^b \nabla^c \Box^d A$ for each $(a,b)$ and $(c,d)$ in $I$ and $J$ from $\Rightarrow A$, in a manner similar to the proof of \ref{lem:vdash}, and then cut them to get $\Rightarrow \Delta$.

	Now back to the stronger form, using induction on the length of $\mathcal{D}$, we can assume for any $\Gamma_1'$, $\Gamma_2'$, $\Delta'$ and $\mathcal{D}'$, where $\mathcal{D}'$ is a $\mathbf{LDL}^*_\rightsquigarrow(L)$ proof for $\Gamma_1' , \Gamma_2' \Rightarrow \Delta'$ and $h(\mathcal{D}')<h(\mathcal{D})$, there is a formula $C_{\langle \Gamma_1';\Gamma_2';\Delta' \rangle}$, more concisely $C'$, and a finite multi-set of formulas $\Pi'$ ($= N(\Gamma_1', f)$ for some proper f) such that
	\begin{enumerate}[label=(\arabic*')]
		\item $\vdash \Pi' \Rightarrow C'$,
		\item $\vdash \Gamma_2' , C' \Rightarrow \Delta'$,
		\item $P(C') \subseteq P(\Gamma_1') \cap P(\Gamma_2',\Delta')$, and
		\item for any $B$ in $\Pi'$ there is $A$ in $\Gamma_1'$ such that $B = \nabla^{n_0} \Box^{n_1} \dots \nabla^{n_{k-1}} \Box^{n_k} A$ for some finite sequence of natural numbers $n_1,...,n_k$.
	\end{enumerate}
	Now construct proper $C$ and $\Pi$ for different cases for the last rule of $\mathcal{D}$. In each case, the immediate sub-trees of $\mathcal{D}$ are denoted by $\mathcal{D}_i (i \in \{0,1\})$.
	\begin{enumerate}
		\item ($Id$) We have $\Gamma_1,\Gamma_2 = \Delta = A$.
		\begin{enumerate}
		...
		\begin{enumerate}
			...
			\item If $\Gamma_1 = \Gamma_1' , \nabla^n (A \lor B)$ and $\Gamma_2 = \Gamma_2'$, let $C_1 = C_{\langle\Gamma_1',\nabla^n A;\Gamma_2';\Delta\rangle}$ and $C_2 = C_{\langle\Gamma_1',\nabla^n B;\Gamma_2';\Delta\rangle}$, and then take $C = C_1 \lor C_2$.
			From IH, by $R\lor_1$ and $R\lor_2$ we can derive $N(\Gamma_1',\nabla^n A,f_1) \Rightarrow C_1 \lor C_2$ and $N(\Gamma_1',\nabla^n B,f_2) \Rightarrow C_1 \lor C_2$ respectively, to which we apply $L\vee$ to get to $N(\Gamma_1',\nabla^n (A \lor B),f) \Rightarrow C_1 \lor C_2$ for some $f$.
			We have $\Gamma_2' , C_1 \lor C_2 \Rightarrow \Delta$ from IH and $L\lor$.
			From IH, we also have $P(C_1) \subseteq P(\Gamma_1' , \nabla^n A) \cap$ $P(\Gamma_2' , \Delta)$ and $P(C_2) \subseteq P(\Gamma_1' , \nabla^n B) \cap P(\Gamma_2' , \Delta)$. Just like the previous case, we can deduce that $P(C_1 \lor C_2) \subseteq P(\Gamma_1' , \nabla^n (A \land B)) \cap P(\Gamma_2' , \Delta)$.
		\end{enumerate}

		\item[10,11.] ($R\lor_i$, {\small$i \in \{1,2\}$}) $\mathcal{D}$ proves $\Gamma_1 , \Gamma_2 \Rightarrow A_1 \lor A_2$ and has a sub-proof for $\Gamma_1 , \Gamma_2 \Rightarrow A_i$. Take $C = C_{\langle\Gamma_1;\Gamma_2;A_i\rangle}$. Then we have $N(\Gamma_1,f) \Rightarrow C$ from IH and $\Gamma_2 , C \Rightarrow A_1 \lor A_2$ from IH and $R\lor_i$.
		From IH, we also have $P(C) \subseteq P(\Gamma_1) \cap P(\Gamma_2 , A_1 \lor A_2)$, as was justified before.
		\setcounter{enumi}{11}
		...
		\begin{enumerate}
			...
			\item If $\Gamma_1 = \Gamma_1' , \nabla^{n+1} (A \rightsquigarrow B)$ and $\Gamma_2 = \Gamma_2'$, let $C_1 = C_{\langle\Gamma_2';\Gamma_1';\nabla^n A\rangle}$ and $C_2 = C_{\langle\Gamma_1',\nabla^n B;\Gamma_2';\Delta\rangle}$, and take $C = \nabla (C_1 \rightsquigarrow C_2)$.
			From IH we have $\Gamma_1' , C_1 \Rightarrow \nabla^n A$. Also from IH, with a $Lw$ to add $C_1$ to the left, we have $N(\Gamma_1',\nabla^n B,f_1) , C_1 \Rightarrow C_2$. By applying $L\rightsquigarrow$ we get $N(\Gamma_1,\nabla^{n+1}(A \rightsquigarrow B),f) , C_1 \Rightarrow C_2$ for some $f$. By lemma \ref{lem:l-nabla-box} and $Cut$ we can add a $\nabla\Box$ to all formulas in $N(\Gamma_1',\nabla^{n+1}(A \rightsquigarrow B),f)$, so we can move $C_1$ to the right by $R\rightsquigarrow$. With $N'$ we will have $N(\Gamma_1',\nabla^{n+1}(A \rightsquigarrow B),f') , \Rightarrow \nabla (C_1 \rightsquigarrow C_2)$ for some $f'$.
			From IH we also have $N(\Gamma_2',f_2) \Rightarrow C_1$ and $\Gamma_2' , C_2 \Rightarrow \Delta$. With proper applications of $Lw$, we can equalize their contexts to $N(\Gamma_2',f_2) , \Gamma_2' \Rightarrow C_1$ and $N(\Gamma_2',f_2) , \Gamma_2' , C_2 \Rightarrow \Delta$, from which we then derive $N(\Gamma_2',f_2) , \Gamma_2' , \nabla (C_1 \rightsquigarrow C_2) \Rightarrow \Delta$ by an application of $L\rightsquigarrow$. We also have from IH $P(C_1) \subseteq P(\Gamma_2') \cap P(\Gamma_1' , \nabla^n A)$ and $P(C_2) \subseteq P(\Gamma_1' , \nabla^n B) \cap P(\Gamma_2' , \Delta)$. Then $P(\nabla (C_1 \rightsquigarrow C_2)) \subseteq P(\Gamma_1' , \nabla^{n+1} (A \rightsquigarrow B)) \cap P(\Gamma_2' , \Delta)$.
		\end{enumerate}

		\item ($N'$) $\mathcal{D}$ proves $\nabla \Gamma_1 , \nabla \Gamma_2 \Rightarrow \nabla \Delta$ and has a sub-proof for $\Gamma_1 , \Gamma_2 \Rightarrow \Delta$. Just take $C = C(\Gamma_1;\Gamma_2;\Delta)$ and apply $N'$ on the sequents from IH. todo The variable condition is also trivial from IH.
		
		\item ($L$) $\mathcal{D}$ proves $\Gamma_1' , \Gamma_2' , \nabla A \Rightarrow \Delta$ and has a sub-proof for $\Gamma_1' , \Gamma_2' , A \Rightarrow \Delta$.
		\begin{enumerate}
			\item If $\Gamma_1 = \Gamma_1'$ and $\Gamma_2 = \Gamma_2' , A$, take $C = C_{\langle\Gamma_1';\Gamma_2',A;\Delta\rangle}$. Then we have $\Pi \Rightarrow C$ by IH and $\Gamma_2' , \nabla A \Rightarrow \Delta$ by IH and $L$. From IH, we can also deduce $P(C) \subseteq P(\Gamma_1') \cap P(\Gamma_2',\nabla A,\Delta)$, since $\nabla$ does not introduce new propositional variables.
